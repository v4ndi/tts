...
from transform import mask_from_seq_lengths
...
class TransformerTTS(nn.Module):
    def __init__(self, device="cuda", num_encoder_blocks=3, num_decoder_blocks=3):
        ...
        self.encoder_blocks = nn.ModuleList([
            EncoderBlock() for _ in range(num_encoder_blocks)
        ])

        # Decoder blocks
        self.decoder_blocks = nn.ModuleList([
            DecoderBlock() for _ in range(num_decoder_blocks)
        ])
    ...
    def forward(self, text, text_len, mel, mel_len):
        ...
        self.src_key_padding_mask = torch.zeros(
            ...
            ~mask_from_seq_lengths(text_len, max_length=S),
            float("-inf")
        )
        ...
        self.tgt_key_padding_mask = torch.zeros(
            ...
            ~mask_from_seq_lengths(mel_len, max_length=TIME),
            float("-inf")
        )
        ...
        # Positional encoding
        pos_codes = self.pos_encoding(torch.arange(hp.max_mel_time).to(mel.device))  # (MAX_S_TIME, E)
        S = text_x.shape[1]
        ...
        text_x = self.norm_memory(text_x)  # Normalize memory
